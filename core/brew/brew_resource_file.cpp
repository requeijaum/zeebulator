...
bool ParseBrewResourceDirectory(const std::vector<uint8_t>& file, BrewResourceDirectory* out) {
  ...
  if (records_start < 0x20 || records_start > file.size()) return false;
  ...
  if (out->offset_table != records_start + records_bytes) return false;
  if (static_cast<size_t>(out->offset_table) + static_cast<size_t>(out->offset_count) * 4 >
      file.size()) {
    return false;
  }
  return true;
...
bool ReadBrewResourceRecord(const std::vector<uint8_t>& file, const BrewResourceDirectory& dir,
  ...
  for (uint16_t i = 0; i < dir.record_count; ++i) {
    ...
    const uint32_t end = (index + 1 < dir.offset_count)
                             ...
                             : dir.data_end;
    if (start >= end || end > file.size()) return false;
    *out_start = start;
    *out_size = end - start;
    return true;
  }
...
bool ReadBrewResourceString(const std::vector<uint8_t>& file, const BrewResourceDirectory& dir,
  ...
  uint32_t start = 0;
  uint32_t size = 0;
  if (!ReadBrewResourceRecord(file, dir, /*type=*/1, id, /*type_match_any=*/true, &start, &size)) {
    return false;
  }
